...
WxService.prototype.setAuthorizerOption = function(options, cb){
  ...
  if(!component_access_token || !json.component_appid || !json.authorizer_appid || !json.option_name || (json.option_value != 0 && !json.option_value)) return cb({errMsg: "params invalid", info:options});  
  request.post({
...
function fetchComponentAccessToken (options, cb){
  ...
  getAccessTokenFromRedis.call(self, function(err, ret){
    ...
    if(ret && ret.ttl && ret.component_access_token){
      if(ret.ttl < 60){
        getAccessTokenFromWX.call(this, options, function(err, obj){
          if(err) {
            console.error(err);
            return cb(err);
          }
