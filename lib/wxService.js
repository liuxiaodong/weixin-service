...
var WxService = function(options){
  ...
  if(typeof options.attrNameProcessors === 'function') {
    attrNameProcessors = options.attrNameProcessors;
  }else if(typeof options.attrNameProcessors === 'string'){
    attrNameProcessors = defaultFormatStr[options.attrNameProcessors || 'underscored'];
  }

  if(!attrNameProcessors) attrNameProcessors = defaultFormatStr.underscored;
...
WxService.prototype.preAuthCode = function(options, cb){
  ...
  var fun = function(cb){
    request.post({
      ...
      if(err) return cb(err);
      var arr = parseBody(body);
      if(arr[0]) return cb(arr[0]);
      body = arr[1];
      if(self.redisClient){
